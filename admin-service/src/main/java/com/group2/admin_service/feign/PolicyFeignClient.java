...
public interface PolicyFeignClient {
	...
	@GetMapping("/api/admin/user-policies/{userId}")
	java.util.List<Object> getUserPolicies(@PathVariable("userId") Long userId);

	@GetMapping("/api/admin/user-policies/all")
	java.util.List<Object> getAllUserPolicies();


	@PutMapping("/api/admin/policies/{id}/cancel")
	Object cancelPolicy(@PathVariable("id") Long id);
}
